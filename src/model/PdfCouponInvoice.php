<?php
    ...
    class PdfCouponInvoice extends \FPDF
	{
		...
		public function getData(\src\objects\Coupon $coupon, $invoiceID, Package $package )
		{
			$this->data = $coupon;
            /**
             * @var Package $package
			...
			$this->prepaid   = $coupon->payment;

			$this->date = ($package->getPackageDeadline() === NULL ) ? date("d.m.Y", time() + (60*60*24*365)) : date("d.m.Y", strtotime($package->getPackageDeadline()));
		...
		public function Header()
		{
			...
			$this->MultiCell( 80, 5,
				ucfirst( utf8_decode( html_entity_decode( $this->data->firstname ) ) ) . ' ' .
				ucfirst( utf8_decode( html_entity_decode( $this->data->lastname ) ) ) . "\n" .
				ucfirst( utf8_decode( html_entity_decode( $this->data->street ) ) ) . ' ' . "\n" .
				utf8_decode( html_entity_decode( $this->data->postcode ) ) . ' ' .
				ucfirst( utf8_decode( html_entity_decode( $this->data->city ) ) ), 0, '', false );
		...
		public function TableBody( $header = array( 'Position', 'Artikel / Leistung', 'Anzahl', 'Preis pro Stunde' ) )
		{
            ...
            $this->Cell( $w[1], 12, utf8_decode( "Gutschein für Liveunterricht bei " . $this->package->getPackageName() ), 'B' );
            $this->Cell( $w[2], 12, utf8_decode( $this->data->lessonCount . " x 60 Minuten" ), 'B' );
            $this->Cell( $w[3], 12,
                str_replace( '.', ',', number_format( ( $price / 1.19 ), 2 ) ) . ' ' . chr( 128 ) . '   ',
                'B', 0, 'R' );
            $this->Ln();
            $count ++;
            $result = $price * $this->data->lessonCount;
